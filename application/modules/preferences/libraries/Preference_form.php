...
class Preference_form
{
	...
	function __construct($config = array())
	{
		...
		// Load language files
		$this->CI->lang->load('preferences/preferences');
	...
	function _setup_fields()
	{
		...
		foreach($this->field as $field => $data)
		{
			...
			{
				$this->field[$field]['label'] = ucwords(str_replace('_',' ',$field));
			}
	...
	function display($print = FALSE)
	{
		...
		if(count($this->group) != 0)
		{
			...
			else
			{
				...
				return $this->CI->load->view("preferences/field_groups",$data, !$print);
			}
		}
	...
	function _display_fields($print,$group_id = NULL)
	{
		...
		$this->CI->form_validation->set_fields($form_fields);
		...
		if( ! $this->CI->input->post('submit'))
		{
			foreach($fields_to_show as $key => $value)
			{
				$this->CI->form_validation->$key = $this->CI->preference->item($key);
			}
		}
		...
		if ($this->CI->form_validation->run() === FALSE)
		{
			...
			return $this->CI->load->view("preferences/form_preference_fields",$data,!$print);
		}
		else
		{
			// SAVE FORM CONTENTS
	...
	function _field_text($key)
	{
		...
		$params['id'] = $key;
		$params['class'] = 'text';
		$params['value'] = $this->CI->form_validation->{$key};
	...
	function _field_boolean($key)
	{
		...
		// Set checked status
		if ($this->CI->form_validation->{$key})
		{
			$params['checked'] = TRUE;
		}
		...
		// Set checked status
		$params['checked'] = ( ! $this->CI->form_validation->{$key} ? TRUE : FALSE);
	...
	function _field_dropdown($key)
	{
		...
		return form_dropdown($key,$options,$this->CI->form_validation->{$key},$this->field[$key]['params']);
	}
