...
class Userlib
{
    ...
    function __construct()
    {
		...
		$this->CI->lang->load('auth/userlib');
	...
	function is_user()
	{
		...
		if($this->CI->session)
		{
			...
			if($this->CI->config->item('sess_use_database') === TRUE)
			{
			...
			else
			{
				...
				{
					case 'email':
						$check = $this->CI->session->userdata('email');
						break;

					default:
						$check = $this->CI->session->userdata('username');
				}
				$result = $this->CI->user_model->validateLogin($check,$CI->session->userdata('password'));
