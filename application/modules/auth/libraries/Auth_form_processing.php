...
class Auth_form_processing
{
	...
	function _register()
	{
		...
		$data['user_profiles']['first_name'] = $this->CI->input->post('first_name');
		$data['user_profiles']['last_name'] = $this->CI->input->post('last_name');
		$data['user_profiles']['advisor'] = $this->CI->input->post('advisor');
		$data['user_profiles']['parent_email1'] = $this->CI->input->post('parent_email1');
		$data['user_profiles']['parent_email2'] = $this->CI->input->post('parent_email2');
		...
		else
		{
			...
			redirect('welcome/student/index','location');
		}
	}
	...
	function register_form($container)
	{
		...
		$config = array(
						...
						array(
						        ...
						        'rules'=>"trim|required|max_length[255]|valid_email|callback_spare_email"
						        ),
						array(
						        'field'=>'password',
		...
		if ( $this->CI->form_validation->run() == FALSE )
		{
            ...
            $where = array('group'=>'5');
            $teachers = $this->CI->user_model->getUsers($where);
		...
		else
		{
			...
			// check email if it contains school email
			
			if($this->spare_workemail($email)==FALSE)
			{
				// this email is not school email, so redirect with message
				flashMsg('error','You must use your school email address.');
...
/*
    * This will be used in a call back form_validation rule to check if user's email has certain 
    * email address which is in a preference.
    */
    public function email_check($email)
    {
        //$name = $this->preference->item('company_name');
        $findme='canacad.ac.jp';// in future get this from preference
        $pos = strpos($email,$findme);
        if ($pos===FALSE)
        {
            $this->form_validation->set_message('email_check', "The %s field does not have our email.");
            return FALSE;
        }
        else
        {
